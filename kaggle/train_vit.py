...
MODEL_NAME = 'vit_base_patch16_384'   # ViT-Base/16 384分辨率预训练
EPOCHS = 40                           # 训练轮数
BATCH_SIZE = 32                       # 批次大小
IMG_SIZE = 384                        # 输入图片尺寸 (放大以保留细节)
LR = 2e-4                            # [核心优化] ViT 迁移学习需要非常小的学习率
WEIGHT_DECAY = 0.1                    # [核心优化] ViT 容易过拟合，需大权重衰减
DROP_PATH_RATE = 0.1                  # Stochastic Depth
MIXUP_ALPHA = 0.8                     # [核心优化] ViT 强依赖高强度 Mixup
CUTMIX_ALPHA = 1.0                    # [核心优化] ViT 强依赖高强度 Cutmix
WARMUP_EPOCHS = 10                    # [核心优化] ViT 需要更长的预热期 (10 epoch)
EMA_DECAY = 0.9998                    # [核心优化] 模型EMA指数移动平均衰减率
GRAD_ACCUM_STEPS = 2                  # [核心优化] 梯度累积步数，等效batch=64
LAYER_DECAY = 0.65                    # [核心优化] ViT 层级学习率衰减更激进 (0.65)
TTA_ENABLED = True                    # [核心优化] 验证时开启 Test-Time Augmentation
PATIENCE = 20                         # [核心优化] Early Stopping 耐心值
OUT_DIR = 'runs_vit'                  # 输出结果和权重的保存目录
# ==========================================


def get_layer_groups(model):
    """
    [核心优化] 为 ViT 实现层级学习率衰减 (Layer-wise LR Decay / LLRD)
    ViT 结构: patch_embed -> blocks.0~11 -> norm -> head
    底层 Transformer Block 用更小的学习率，顶层用更大的学习率。
    """
    param_groups = []
    
    # ViT-Base 有 patch_embed + 12 个 Transformer blocks + head
    # cls_token 和 pos_embed 归为最底层
    layer_prefixes = ['cls_token', 'pos_embed', 'patch_embed',
                      'blocks.0', 'blocks.1', 'blocks.2', 'blocks.3',
                      'blocks.4', 'blocks.5', 'blocks.6', 'blocks.7',
                      'blocks.8', 'blocks.9', 'blocks.10', 'blocks.11']
    num_layers = len(layer_prefixes) + 1  # +1 for head (norm + head)
    ...
    for i, prefix in enumerate(layer_prefixes):
        ...
        for name, param in model.named_parameters():
            # 精确前缀匹配：参数名必须等于前缀或以 "前缀." 开头
            # 防止 'blocks.1' 误匹配 'blocks.10'/'blocks.11'
            if (name == prefix or name.startswith(prefix + '.')) and param.requires_grad and name not in assigned_params:
                params.append(param)
        ...
        if params:
            param_groups.append({
                'params': params,
                'lr': layer_lr,
                'weight_decay': WEIGHT_DECAY if any(p.dim() >= 2 for p in params) else 0.0
            })
    
    # head 层 (norm + head) 使用完整学习率
    head_params_decay = []
    head_params_no_decay = []
    for name, param in model.named_parameters():
        if name not in assigned_params and param.requires_grad:
            if param.dim() >= 2:  # 权重矩阵加衰减
                head_params_decay.append(param)
            else:  # bias 和 LayerNorm 不加衰减
...
def get_cosine_schedule_with_warmup(optimizer, num_warmup_steps, num_training_steps):
    ...
    ViT 特别需要更长的 warmup 来稳定注意力权重的学习。
    """
...
def get_dataloaders():
    ...
    # [核心优化] ViT 用较强的数据增强 (m9)，因 ViT 本身参数量大容易过拟合
...
def main():
    ...
    # [核心优化] ViT 用高强度 MixUp 和 CutMix 防止过拟合
    mixup_fn = Mixup(
    ...
    # [核心优化] 层级学习率衰减 (LLRD) — ViT 的关键提分技术
    print("配置层级学习率衰减 (LLRD) — 适配 ViT 12层 Transformer Blocks...")
    param_groups = get_layer_groups(model)
    optimizer = optim.AdamW(param_groups)
    
    # [核心优化] Warmup + Cosine Annealing (ViT 需要更长的 warmup)
