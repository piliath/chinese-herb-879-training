...
MODEL_NAME = 'convnext_base'          # 推荐 convnext_base
EPOCHS = 40                           # 训练轮数
BATCH_SIZE = 32                       # 批次大小 (3090显存24G足够)
IMG_SIZE = 384                        # 输入图片尺寸 (放大以保留细节)
LR = 5e-4                            # [核心优化] 迁移学习用较小LR保护预训练权重
WEIGHT_DECAY = 0.05                   # ConvNeXt 推荐较大的权重衰减
DROP_PATH_RATE = 0.2                  # 开启 Stochastic Depth 防止过拟合
WARMUP_EPOCHS = 5                     # [核心优化] 前5轮学习率线性预热
EMA_DECAY = 0.9998                    # [核心优化] 模型EMA指数移动平均衰减率
GRAD_ACCUM_STEPS = 2                  # [核心优化] 梯度累积步数，等效batch=64
LAYER_DECAY = 0.7                     # [核心优化] 层级学习率衰减，底层用更小的LR
TTA_ENABLED = True                    # [核心优化] 验证时开启 Test-Time Augmentation
OUT_DIR = 'runs_convnext'             # 输出结果和权重的保存目录
# ==========================================


def get_layer_groups(model):
    """
    [核心优化] 为 ConvNeXt 实现层级学习率衰减 (Layer-wise LR Decay / LLRD)
    底层 (靠近输入的层) 使用更小的学习率，顶层 (靠近分类头) 使用更大的学习率。
    这是迁移学习中提分最有效的技术之一。
    """
    # ConvNeXt 有 stem + 4 个 stage + head
    param_groups = []
    
    # 分组: stem -> stages.0 -> stages.1 -> stages.2 -> stages.3 -> head
    layer_names = ['stem', 'stages.0', 'stages.1', 'stages.2', 'stages.3']
    num_layers = len(layer_names) + 1  # +1 for head
    
    # 为每一层分配衰减后的学习率
    for i, layer_prefix in enumerate(layer_names):
    ...
    head_params = []
    for name, param in model.named_parameters():
        is_in_backbone = any(name.startswith(prefix) for prefix in layer_names)
        if not is_in_backbone and param.requires_grad:
            head_params.append(param)
    if head_params:
        param_groups.append({
            'params': head_params,
            'lr': LR,
            'weight_decay': 0.0  # 分类头不加权重衰减
        })
...
def get_cosine_schedule_with_warmup(optimizer, num_warmup_steps, num_training_steps):
    """
    [核心优化] 自定义 Warmup + Cosine Annealing 学习率调度器
...
class ModelEMA:
    """
    [核心优化] 模型指数移动平均 (EMA)
    维护一份模型权重的滑动平均，验证时使用EMA模型，通常能白嫖 0.3~1% 的精度。
    """
...
def get_dataloaders():
    ...
    # [核心优化] 使用 timm 的 create_transform，RandAugment 强度适中 (m7 而非 m9)
    # 过强的增强会导致小数据集上欠拟合
    train_transform = create_transform(
        input_size=IMG_SIZE,
        is_training=True,
        color_jitter=0.3,                    # 略微降低颜色抖动
        auto_augment='rand-m7-mstd0.5-inc1', # [核心优化] 降低 RandAugment 强度 m9->m7
        interpolation='bicubic',
        re_prob=0.2,                         # 随机擦除概率降低一点
        re_mode='pixel',
        re_count=1,
    )
...
def get_tta_transforms():
    ...
    在验证时对同一张图片做多种变换，取平均预测，进一步稳定并提升精度。
    """
    return [
        # 原始图
...
def main():
    ...
    # 2. 构建模型 (ConvNeXt 专属优化: 加入 DropPath)
    ...
    # [核心优化] Warmup + Cosine Annealing 学习率调度
    ...
    for epoch in range(EPOCHS):
        ...
        for step, (inputs, targets) in enumerate(pbar):
            ...
            with torch.cuda.amp.autocast():
                ...
                loss = loss / GRAD_ACCUM_STEPS  # [核心优化] 梯度累积：损失除以累积步数
            
            scaler.scale(loss).backward()

            # [核心优化] 每 GRAD_ACCUM_STEPS 步才更新一次权重，等效于更大的batch size
